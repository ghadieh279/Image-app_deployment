...
def load_model():
    with st.spinner("Loading the SmartSort APP..."):
...
def select_folder():
    ...
    folder_path = filedialog.askdirectory(title="Choose a folder", mustexist=True)
    return folder_path
...
def move_images(image_bytes_list, initial_predictions, user_selected_predictions, root_folder, use_combined_folder=False):
    for img_bytes, initial_pred, user_selected_class in zip(image_bytes_list, initial_predictions, user_selected_predictions):
        ...
        if use_combined_folder and abs(first_pred_accuracy - second_pred_accuracy) <= st.session_state['avg']:
...
def main():
    st.title('SmartSort')
    st.markdown("Image Classification App")
